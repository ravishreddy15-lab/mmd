...
export function getPersonalizedRecommendations(params: {
  ...
  const targetStyle = (params.style || "").toLowerCase().trim();
  const targetPace = (params.tempo || "").toLowerCase().trim();
  const targetLength = (params.length || "").toLowerCase().trim();
  ...
  // Filter candidates first if hard constraints apply
  let candidates = [...MOVIES_DATABASE];

  // If user explicitly chose "short" / under 90m runtime, filter out movies longer than 110m
  if (targetLength === "short" || targetLength.includes("90") || targetLength.includes("under")) {
    const under110 = candidates.filter((m) => m.duration_minutes <= 110);
    if (under110.length >= 4) {
      candidates = under110;
    }
  } else if (targetLength === "long" || targetLength.includes("2+") || targetLength.includes("hour")) {
    const over115 = candidates.filter((m) => m.duration_minutes >= 115);
    if (over115.length >= 4) {
      candidates = over115;
    }
  } else if (targetLength === "medium" || targetLength.includes("90-120") || targetLength.includes("standard")) {
    const midRange = candidates.filter((m) => m.duration_minutes >= 80 && m.duration_minutes <= 145);
    if (midRange.length >= 4) {
      candidates = midRange;
    }
  }

  // If genres are provided, prioritize candidates that match at least one genre
  if (targetGenres.length > 0) {
    const genreMatched = candidates.filter((m) =>
      m.genre.some((g) => targetGenres.includes(g.toLowerCase()))
    );
    // If we have enough genre matches, use those exclusively or prioritize them heavily
    if (genreMatched.length >= 4) {
  ...
  const scoredMovies = candidates.map((movie) => {
    ...
    // 1. Direct genre matching (very high weight)
    if (targetGenres.length > 0) {
      const directMatches = movie.genre.filter((g) => targetGenres.includes(g.toLowerCase()));
      if (directMatches.length > 0) {
        score += directMatches.length * 35;
        reasons.push(`${directMatches.join(" & ")} genre`);
      } else {
        score -= 40; // Penalty if none of the requested genres match
      }
    }

    // 2. Exact Length / Runtime scoring
    if (targetLength === "short" || targetLength.includes("90") || targetLength.includes("under")) {
      if (movie.duration_minutes <= 90) {
        score += 55;
        reasons.push(`under 90m runtime (${movie.duration})`);
      } else if (movie.duration_minutes <= 100) {
        score += 35;
        reasons.push(`concise ${movie.duration} runtime`);
      } else if (movie.duration_minutes <= 110) {
        score += 15;
      } else {
        score -= 60;
      }
    } else if (targetLength === "medium" || targetLength.includes("90-120") || targetLength.includes("standard")) {
      if (movie.duration_minutes >= 90 && movie.duration_minutes <= 125) {
        score += 50;
        reasons.push(`ideal ${movie.duration} runtime`);
      } else if (movie.duration_minutes >= 80 && movie.duration_minutes <= 135) {
        score += 25;
      } else {
        score -= 40;
      }
    } else if (targetLength === "long" || targetLength.includes("2+") || targetLength.includes("hour")) {
      if (movie.duration_minutes >= 135) {
        score += 55;
        reasons.push(`epic ${movie.duration} duration`);
      } else if (movie.duration_minutes >= 120) {
        score += 35;
        reasons.push(`expansive ${movie.duration} story`);
      } else {
        score -= 50;
      }
    }
    ...
    if (targetMood) {
      ...
      if (moodMatches.length > 0) {
        ...
        reasons.push(`matches "${targetMood}" mood`);
      }
      ...
      if (targetMood.includes("mind") || targetMood.includes("thought") || targetMood.includes("deep")) {
        if (movieGenresLower.includes("sci-fi") || movieGenresLower.includes("mystery") || movie.director === "Christopher Nolan") {
          score += 25;
          reasons.push("mind-bending narrative twists");
        }
      } else if (targetMood.includes("thrill") || targetMood.includes("intense") || targetMood.includes("excited") || targetMood.includes("action")) {
        if (movieGenresLower.includes("action") || movieGenresLower.includes("thriller") || movieGenresLower.includes("crime")) {
          score += 25;
          reasons.push("high-stakes thrilling energy");
        }
        ...
        if (movieGenresLower.includes("animation") || movieGenresLower.includes("comedy") || movieGenresLower.includes("family") || movieGenresLower.includes("romance")) {
          score += 25;
          reasons.push("heartwarming feel-good tone");
        }
      } else if (targetMood.includes("dark") || targetMood.includes("scare") || targetMood.includes("gritty") || targetMood.includes("horror")) {
        if (movieGenresLower.includes("crime") || movieGenresLower.includes("horror") || movieGenresLower.includes("mystery")) {
          score += 25;
          reasons.push("dark atmospheric tension");
        }
      } else if (targetMood.includes("romantic") || targetMood.includes("love")) {
        if (movieGenresLower.includes("romance") || movieGenresLower.includes("drama")) {
          score += 25;
          reasons.push("captivating romantic chemistry");
        }
      }
    }
    ...
    // 4. Style matching
    if (targetStyle) {
      if (targetStyle === "blockbuster" || targetStyle.includes("spectacle")) {
        if (movie.votes > 800000 || movieGenresLower.includes("action") || movieGenresLower.includes("adventure") || movieGenresLower.includes("sci-fi")) {
          score += 30;
          reasons.push("blockbuster visual scale");
        }
      } else if (targetStyle === "indie" || targetStyle.includes("gem")) {
        if (movie.votes < 850000 || movie.country.includes("France") || movie.country.includes("Japan") || movie.country.includes("South Korea") || movieGenresLower.includes("drama") || movieGenresLower.includes("romance")) {
          score += 30;
          reasons.push("artistic auteur storytelling");
        }
      } else if (targetStyle === "classic" || targetStyle.includes("timeless")) {
        if (movie.year < 2005) {
          score += 35;
          reasons.push(`timeless ${movie.year} classic`);
        } else if (movie.rating >= 8.5) {
          score += 20;
          reasons.push("modern cinematic masterpiece");
        }
      } else if (targetStyle === "hidden") {
        if (movie.votes < 900000 && movie.rating >= 7.8) {
          score += 35;
          reasons.push("critically acclaimed hidden gem");
        }
      }
    }
    ...
    // 5. Pacing matching
    if (targetPace) {
      if (targetPace === "fast" || targetPace.includes("fast")) {
        if (movieGenresLower.includes("action") || movieGenresLower.includes("thriller") || movieGenresLower.includes("animation") || movie.duration_minutes <= 125) {
          score += 25;
          reasons.push("fast-paced momentum");
        }
      } else if (targetPace === "slow" || targetPace.includes("slow") || targetPace.includes("burn")) {
        if (movieGenresLower.includes("mystery") || movieGenresLower.includes("drama") || movie.duration_minutes >= 130) {
          score += 25;
          reasons.push("deliberate slow-burn atmosphere");
        }
      } else if (targetPace === "moderate") {
        score += 15;
      }
    }

    // 6. Rating & Critical Acclaim
    ...
    const match_percentage = Math.min(99, Math.max(78, Math.round(score)));
    const reason = reasons.length > 0
      ? `Selected for your ${reasons.slice(0, 3).join(", ")}.`
      : `Acclaimed pick (${movie.rating}/10) directed by ${movie.director} (${movie.duration}).`;
...
export function getQuizRecommendations(answers: any): RecommendationResult[] {
  ...
  const length = answers.length || answers.movie_length || "";
