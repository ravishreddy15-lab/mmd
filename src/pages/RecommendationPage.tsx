...
export default function RecommendationPage() {
  ...
  const hasActiveFilters = Boolean(moodParam || genresParam || styleParam || paceParam || lengthParam);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
        <div className="text-center space-y-3 animate-fade-in-down">
          ...
          <p className="text-zinc-400 text-base sm:text-lg max-w-2xl mx-auto">
            {hasActiveFilters ? (
              "Tailored precisely to your quiz responses and cinematic preferences:"
            ) : (
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
              {moodParam && (
                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-purple-950/70 border border-purple-500/40 text-purple-200">
                  🎭 Mood: <span className="capitalize">{moodParam}</span>
                </span>
              )}
              {genresParam && (
                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-purple-950/70 border border-purple-500/40 text-purple-200">
                  🎬 Genres: <span>{genresParam.split(",").join(", ")}</span>
                </span>
              )}
              {lengthParam && (
                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-950/70 border border-emerald-500/40 text-emerald-300">
                  ⏱️ Length: <span>{lengthLabels[lengthParam] || lengthParam}</span>
                </span>
              )}
              {styleParam && (
                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-blue-950/70 border border-blue-500/40 text-blue-300">
                  ✨ Style: <span>{styleLabels[styleParam] || styleParam}</span>
                </span>
              )}
              {paceParam && (
                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-amber-950/70 border border-amber-500/40 text-amber-300">
                  ⚡ Pace: <span>{paceLabels[paceParam] || paceParam}</span>
                </span>
              )}
            </div>
          )}
                  ...
                  <div className="group rounded-xl border border-zinc-800 bg-zinc-900/80 overflow-hidden hover:border-purple-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-purple-950/20">
                    ...
                    <div className="p-4 space-y-2.5">
                      ...
                      <div className="flex items-center gap-2 text-xs text-zinc-400">
                        ...
                        <span className={cn(
                          "font-medium",
                          lengthParam === "short" && rec.movie.duration_minutes <= 95 ? "text-emerald-400" :
                          lengthParam === "long" && rec.movie.duration_minutes >= 130 ? "text-emerald-400" :
                          lengthParam === "medium" && rec.movie.duration_minutes >= 90 && rec.movie.duration_minutes <= 125 ? "text-emerald-400" :
                          "text-zinc-300"
                        )}>
                          {rec.movie.duration}
                        </span>
                      ...
                      {rec.reason && (
                        <p className="text-xs text-zinc-300 mt-1 line-clamp-2 leading-relaxed bg-zinc-950/60 p-2.5 rounded-lg border border-purple-900/30">
                          {rec.reason}
                        </p>
                      )}
                    </div>
